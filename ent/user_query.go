...
func (uq *UserQuery) QueryAuthentication() *AuthQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAuthentication: uq.withAuthentication.Clone(),
...
func (uq *UserQuery) WithAuthentication(opts ...func(*AuthQuery)) *UserQuery {
	query := &AuthQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withAuthentication = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAuthentication; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Authentication = []*Auth{}
		}
		query.withFKs = true
		query.Where(predicate.Auth(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AuthenticationColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_authentication
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_authentication" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_authentication" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Authentication = append(node.Edges.Authentication, n)
		}
	}
