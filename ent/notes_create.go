...
func (nc *NotesCreate) SetFImage(s string) *NotesCreate {
	nc.mutation.SetFImage(s)
	return nc
}
...
func (nc *NotesCreate) SetUpdatedAt(t time.Time) *NotesCreate {
	nc.mutation.SetUpdatedAt(t)
	return nc
}

// SetNillableUpdatedAt sets the "updated_at" field if the given value is not nil.
func (nc *NotesCreate) SetNillableUpdatedAt(t *time.Time) *NotesCreate {
	if t != nil {
		nc.SetUpdatedAt(*t)
	}
	return nc
...
func (nc *NotesCreate) SetNillableID(u *uuid.UUID) *NotesCreate {
	if u != nil {
		nc.SetID(*u)
	}
	return nc
...
func (nc *NotesCreate) check() error {
	if _, ok := nc.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`ent: missing required field "Notes.title"`)}
	}
	if v, ok := nc.mutation.Title(); ok {
		if err := notes.TitleValidator(v); err != nil {
			return &ValidationError{Name: "title", err: fmt.Errorf(`ent: validator failed for field "Notes.title": %w`, err)}
		}
	}
	if _, ok := nc.mutation.Body(); !ok {
		return &ValidationError{Name: "body", err: errors.New(`ent: missing required field "Notes.body"`)}
	}
	if v, ok := nc.mutation.Body(); ok {
		if err := notes.BodyValidator(v); err != nil {
			return &ValidationError{Name: "body", err: fmt.Errorf(`ent: validator failed for field "Notes.body": %w`, err)}
		}
	}
	if _, ok := nc.mutation.Tags(); !ok {
		return &ValidationError{Name: "tags", err: errors.New(`ent: missing required field "Notes.tags"`)}
	}
	if _, ok := nc.mutation.Slug(); !ok {
		return &ValidationError{Name: "slug", err: errors.New(`ent: missing required field "Notes.slug"`)}
	}
	if v, ok := nc.mutation.Slug(); ok {
		if err := notes.SlugValidator(v); err != nil {
			return &ValidationError{Name: "slug", err: fmt.Errorf(`ent: validator failed for field "Notes.slug": %w`, err)}
		}
	}
	if _, ok := nc.mutation.FImage(); !ok {
		return &ValidationError{Name: "f_image", err: errors.New(`ent: missing required field "Notes.f_image"`)}
	}
	if v, ok := nc.mutation.FImage(); ok {
		if err := notes.FImageValidator(v); err != nil {
			return &ValidationError{Name: "f_image", err: fmt.Errorf(`ent: validator failed for field "Notes.f_image": %w`, err)}
		}
	}
	if _, ok := nc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "Notes.created_at"`)}
	}
	return nil
}
...
func (nc *NotesCreate) createSpec() (*Notes, *sqlgraph.CreateSpec) {
	...
	if value, ok := nc.mutation.FImage(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: notes.FieldFImage,
		})
		_node.FImage = value
	}
