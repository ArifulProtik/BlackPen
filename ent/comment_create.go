...
func (cc *CommentCreate) SetNoteID(u uuid.UUID) *CommentCreate {
	cc.mutation.SetNoteID(u)
	return cc
}
...
func (cc *CommentCreate) check() error {
	...
	if _, ok := cc.mutation.NoteID(); !ok {
		return &ValidationError{Name: "note_id", err: errors.New(`ent: missing required field "Comment.note_id"`)}
	}
	if _, ok := cc.mutation.CreatedAt(); !ok {
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.NoteID(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeUUID,
			Value:  value,
			Column: comment.FieldNoteID,
		})
		_node.NoteID = value
	}
